...
export default {
    ...
    methods: {
         createAccount() {
            console.log('log values', this.createAccountForm);
            try {
                this.$axios.get('api/airlock/csrf-cookie')
                .then(response => {
                    this.$axios.post('api/register', this.createAccountForm);
                })
                this.errorMessage || false;
                console.log('log values ->', this.createAccountForm);
            } catch (err) {
                this.errorMessage = true;
                console.log('log values -> 2', this.createAccountForm);
            }
        },
